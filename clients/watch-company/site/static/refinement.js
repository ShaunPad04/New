...
(() => {
  ...
  nav.innerHTML=`<div class="reference-menu-links"><section><h2>THE COLLECTION</h2><a href="/collection/?sort=recent">Latest arrivals</a><a href="/collection/">Shop all watches</a>${menuBrands.map(brand=>`<a href="/collection/?brand=${encodeURIComponent(brand)}">${brand}</a>`).join('')}<a href="/collection/?brand=Cartier">Cartier</a><a href="/collection/?brand=Omega">Omega</a></section><section><h2>THE WATCH CLUB</h2><a href="/services/">Sell your watch</a><a href="/part-exchange/">Part-exchange</a><a href="/about/">Our story</a><a href="/about/">Meet the specialists</a><a href="/journal/">Blogs</a><a href="/contact/">Visit & contact</a><a href="/collection/#search">Search watches</a></section></div><div class="reference-menu-images">${menuWatches.filter(Boolean).map(w=>`<a class="menu-watch-panel" href="/collection/?brand=${encodeURIComponent(w.brand)}"><div class="menu-watch-title"><strong>${w.brand}</strong><span>Explore the collection</span></div><img src="${w.image}" alt="${w.brand} ${w.name}" loading="lazy"><span class="menu-watch-model">${w.name}</span></a>`).join('')}</div>`;
  ...
  if(home){
    ...
    if(selection){selection.querySelector(':scope > .eyebrow')?.remove();const study=document.createElement('section');study.className='editorial-watch-study';study.innerHTML='<div class="study-image"><img src="/assets/rolex-being-looked-at.png" alt="A Rolex Day-Date examined by hand" loading="lazy" width="2048" height="877" decoding="async"></div><div class="study-copy"><p class="eyebrow">A CLOSER LOOK</p><h2>The feeling<br>of finding it.</h2><p>Some watches announce themselves. Others reveal their character slowly, detail by detail. Take your time with a piece selected for the way it feels on the wrist.</p><a class="outline" href="/collection/">Explore the collection ↗</a></div>';selection.after(study);}
