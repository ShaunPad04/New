...
(() => {
  ...
  const allBrands=[...new Set(WATCHES.map(w=>w.brand))]
    .sort((a,b)=>(stocked.has(b)-stocked.has(a))||a.localeCompare(b));
  const menuWatches=menuBrands.map(brand=>WATCHES.find(w=>w.brand===brand));
  nav.innerHTML=`<div class="reference-menu-links"><section><h2>THE COLLECTION</h2><a href="/collection/?sort=recent">Latest arrivals</a><a href="/collection/">Shop all watches</a></section><section class="menu-brands-all"><h2>BRANDS</h2><div class="menu-brand-grid">${allBrands.map(brand=>`<a href="${stocked.has(brand)?'/collection/':'/archive/'}?brand=${encodeURIComponent(brand)}">${brand}${stocked.has(brand)?'':'<i>sold archive</i>'}</a>`).join('')}</div></section><section><h2>THE WATCH CLUB</h2><a href="/services/">Sell your watch</a><a href="/part-exchange/">Part-exchange</a><a href="/about/">Our story</a><a href="/about/">Meet the specialists</a><a href="/journal/">Blogs</a><a href="/contact/">Visit & contact</a><a href="/collection/#search">Search watches</a></section></div><div class="reference-menu-images">${menuWatches.filter(Boolean).map(w=>`<a class="menu-watch-panel" href="/collection/?brand=${encodeURIComponent(w.brand)}"><div class="menu-watch-title"><strong>${w.brand}</strong><span>Explore the collection</span></div><img src="${w.image}" alt="${w.brand} ${w.name}" loading="lazy"><span class="menu-watch-model">${w.name}</span></a>`).join('')}</div>`;
